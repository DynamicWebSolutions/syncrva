...
function widget_register_new_user( $user_login, $user_email ) {
	...
	$fromEmail = get_site_emailId();
		...
		$search_array1 = array('[#user_name#]','[#user_login#]','[#user_password#]','[#site_name#]','[#site_login_url#]','[#site_login_url_link#]');
		$replace_array = array($user_login,$user_login,$user_pass,$store_name,$store_login,$store_login_link);
		$client_message = str_replace($search_array,$replace_array,$client_message);
		$client_message = str_replace($search_array1,$replace_array,$client_message);
		//sendEmail($fromEmail,$fromEmailName,$user_email,$userName,$subject,$client_message,$extra=''); //To clidne email
		$headers  = 'MIME-Version: 1.0' . "\r\n";
		$headers .= 'Content-type: text/html; charset=UTF-8' . "\r\n";
		// Additional headers
		$headers .= 'From: '.$fromEmailName.' <'.$fromEmail.'>' . "\r\n";
		if(wp_mail( $user_email, $subject, $client_message, $headers ))
		{
			
		}else
		{
			mail( $user_email, $subject, $client_message, $headers );
...
if(isset($_REQUEST['widgetptype']) == 'register' && (isset($_POST['new_user_registration']) && wp_verify_nonce($_POST['new_user_registration'],'user_registration')))
{
	...
	$user_email = '';
		...
		if (!$resp->is_valid ) {
			echo "<script type='text/javascript'>alert('Invalid recaptcha');</script>";
			exit;
		} 
		}
  ...
  <ul class="xoxo blogroll">
    <?php 
				$authorlink = get_author_posts_url($current_user->ID);
					...
					echo apply_filters('templ_login_widget_editprofilelink_filter','<li><a href="'.$site_url.$op.'ptype=profile'.'">'.EDIT_PROFILE_PAGE_TITLE.'</a></li>');
					echo apply_filters('templ_login_widget_editprofilelink_filter','<li><a href="'.$site_url.$op.'ptype=profile'.'">'.CHANGE_PW_TEXT.'</a></li>');
					...
					echo apply_filters('templ_login_widget_editprofilelink_filter','<li><a href="'.$site_url.$op.'ptype=post_listing'.'">'.__('Add place','templatic').'</a></li>');
					}
					if(get_option('ptthemes_add_event_nav')=='Yes'){
					echo apply_filters('templ_login_widget_editprofilelink_filter','<li><a href="'.$site_url.$op.'ptype=post_event'.'">'.__('Add Event','templatic').'</a></li>');
					}
...
$cwidget(document).ready(function(){
	...
	//On key press
	$cwidget('#widget_user_rlogin').keyup(validate_widget_your_ulogin);
	$cwidget('#widget_user_remail').keyup(validate_widget_your_remail);
  ...
  <form name="loginwidgetform" id="loginwidgetform" action="#login_widget" method="post" >
    ...
    <div class="form_row">
      <label>
        <?php _e('Username','templatic');?>
        <span>*</span></label>
      <input name="log" id="widget_user_login" type="text" class="textfield" />
      <span id="user_login_info"></span> </div>
    <div class="form_row">
      <label>
        <?php _e('Password','templatic');?>
        <span>*</span></label>
      <input name="pwd" id="widget_user_pass" type="password" class="textfield" />
      <span id="your_pass_info"></span> </div>
  ...
  <p class="forgot_link"> <a href="javascript:void(0);showhide_register();" class="lw_new_reg_lnk">
    <?php _e('New User? Register Now','templatic');?>
    </a> <br />
  ...
  <?php
			...
			?>
  <h4>
    <?php _e('New User Register Here','templatic');?>
  </h4>
  <form name="registerform" id="registerform" method="post" action="#login_widget">
    ...
    <div class="form_row clearfix">
      ...
      <span id="your_rname_Info"></span> </div>
    <div class="row_spacer_registration clearfix" >
      <div class="form_row clearfix">
        ...
        <span id="your_remail_Info"></span> </div>
    </div>
    <?php $pcd = explode(',',get_option('ptthemes_captcha_dislay'));	
	 ...
	 <?php wp_nonce_field( 'user_registration', 'new_user_registration' ); ?>
...
<form name="lostpasswordform" id="lostpasswordform" method="post" action="<?php echo $site_url.$op.'ptype=login&amp;action=lostpassword'; ?>">
  <div class="form_row clearfix">
    <label>

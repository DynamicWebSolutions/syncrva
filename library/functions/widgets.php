...
class multi_city extends WP_Widget {
	...
	//Constructor
		...
		$this->WP_Widget('widget_multi_city', __('T &rarr; Multi City Options'), $widget_ops);
	}
...
class homebannerwidget extends WP_Widget {
	...
	//Constructor
		$widget_ops = array('classname' => 'widget Home Banner', 'description' => __('Home page banner slider - you can place this widget instead of map widget in front end.') );
		$this->WP_Widget('widget_homebannerwidget', __('T &rarr; Home Page Banner'), $widget_ops);
	}
	function widget($args, $instance) {
	// prints the widget
		extract($args, EXTR_SKIP);
...
class werecommend extends WP_Widget {
	...
	//Constructor
		...
		$this->WP_Widget('widget_werecommend', __('T &rarr; We Recommend'), $widget_ops);
	}
	function widget($args, $instance) {
	// prints the widget
		extract($args, EXTR_SKIP);
	...
	<p>
	  <label for="<?php echo $this->get_field_id('div_id'); ?>"><?php _e('Enter uniq ID for DIV :');?>
	...
	<p><label for="<?php echo $this->get_field_id('width'); ?>"><?php _e('Banner image width :');?>
	...
	<p><label for="<?php echo $this->get_field_id('height'); ?>"><?php _e('Banner image height :');?>
	...
	<p><label for="<?php echo $this->get_field_id('s1'); ?>"><?php _e('Banner Slider Image 1 full URL(ex.http://templatic.com/images/banner1.png)  :');?>
	<input type="text" class="widefat" id="<?php echo $this->get_field_id('s1'); ?>" name="<?php echo $this->get_field_name('s1'); ?>" value="<?php echo attribute_escape($s1); ?>"></label>
	</p>
	<p><label for="<?php echo $this->get_field_id('s1link'); ?>"><?php _e('Banner Slider Image 1 Link (ex.http://templatic.com)  :');?>
	<input type="text" class="widefat" id="<?php echo $this->get_field_id('s1link'); ?>" name="<?php echo $this->get_field_name('s1link'); ?>" value="<?php echo attribute_escape($s1link); ?>"></label>
...
/* Latest Places/Events Custom Taxonmy  -->  Listview BOC*/
					...
					<div class="post_content">
						...
						<!-- Displat author details and date and etc -->
						 <div class="post-meta listing_meta">
							...
							<label><?php echo BY;?></label> <span class="post-author"> <a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>" title="Posts by <?php the_author(); ?>">
							<?php the_author(); ?>
							</a> </span>
							<?php }  ?>
							<?php if(templ_is_show_listing_date()){?>
							<label><?php echo on;?></label>  <span class="post-date">
...
/* Latest Places/Events Custom Taxonmy  -->  Listview EOC*/

/* Latest Places/Events Custom Taxonmy  -->  Gridview BOC*/
...
/* Latest Places/Events Custom Taxonmy  -->  Gridview EOC*/
...
class upComingEvents extends WP_Widget {
	...
	//Constructor
		...
		$this->WP_Widget('upComingEvents', __('T &rarr; Upcoming Events'), $widget_ops);
	}
...
<p>
  <label for="<?php echo $this->get_field_id('more_title'); ?>"><?php _e('More link name:');?>
...
<p>
  <label for="<?php echo $this->get_field_id('more_link'); ?>"><?php _e('More link URL:');?>
...
class eventwidget extends WP_Widget {
	...
	//Constructor
		...
		$this->WP_Widget('eventwidget', __('T &rarr; Latest Post / Places / Event'), $widget_ops);
	}
...
class spotlightpost extends WP_Widget {
	...
	//Constructor
		...
		$this->WP_Widget('spotlight_post', __('T &rarr; Featured Video'), $widget_ops);
	}
...
class widget_listing_link extends WP_Widget {
	...
	//Constructor
		...
		$this->WP_Widget('listing_link', __('T &rarr; Login & Add place widget'), $widget_ops);
	}
    ...
    ?>
     <li><a href="<?php echo site_url();?>/?ptype=post_listing"><?php _e('Add group','templatic');?></a></li>
	 <?php }
...
class neighborhood_bymiles extends WP_Widget {
	...
	//Constructor
		$widget_ops = array('classname' => 'widget In the neighborhood by miles', 'description' => __('In the neighborhood Post List by miles ') );
		$this->WP_Widget('neighborhood_bymiles', __('PT &rarr; In the neighborhood by miles'), $widget_ops);
	}
	...
	function widget($args, $instance) {
		...
		//$qry = "SELECT post_id FROM $postcode WHERE post_id!=".$post->ID." and post_type='".$current_post_type."' and  (((acos(sin((".$geo_latitude."*pi()/180)) * sin((`Latitude`*pi()/180))+cos((".$geo_latitude."*pi()/180)) * cos((`Latitude`*pi()/180)) * cos(((".$geo_longitude."- `Longitude`)*pi()/180))))*180/pi())*60*1.1515) <= ".$miles;

		$qry = "SELECT pm.post_id FROM $postcode pc, $post_meta pm WHERE pm.post_id!=".$post->ID."  and pc.post_id=pm.post_id and pm.meta_key='post_city_id' and pm.meta_value=$multi_city_id and (((acos(sin((".$geo_latitude."*pi()/180)) * sin((`Latitude`*pi()/180))+cos((".$geo_latitude."*pi()/180)) * cos((`Latitude`*pi()/180)) * cos(((".$geo_longitude."- `Longitude`)*pi()/180))))*180/pi())*60*1.1515) <= ".$miles;
		...
		{
			$arg='post_id='.$res->post_id.'&post_type=event';
			$post_details=get_post($res->post_id);
			$post=$post_details;
			$post_images = bdw_get_images_with_info($res->post_id,'large');
			$attachment_id = $post_images[0]['id'];
			$alt = get_post_meta($attachment_id, '_wp_attachment_image_alt', true);
			$attach_data = get_post($attachment_id);
			$width = 40;
			...
			<li class="clearfix">
            	 <?php if($post_images[0]['file']){
					$crop_image = vt_resize($attachment_id, $post_images[0]['file'], $width, $height, $crop = $is_crop );?>
                   <a  href="<?php echo get_permalink($res->post_id); ?>"><img src="<?php echo $crop_image['url']; ?>" alt="<?php echo get_the_title($res->post_id); ?>" title="<?php echo get_the_title($res->post_id); ?>" class="thumb" /> </a>
            <?php } else { ?>
						 <a  href="<?php echo get_permalink($res->post_id); ?>"><span class="img_available">   <?php _e('Image not available');?> </span></a>
				<?php }?>

					<a href="<?php echo get_permalink($res->post_id); ?>"><?php echo $post_details->post_title; ?></a>
					<?php  if(get_option('ptthemes_disable_rating') == 'no') {  ?>

                    <span class="rating"> <?php echo get_post_rating_star($res->post_id);?></span>
					<?php } ?>
	...
	<p>
	 <label for="<?php echo $this->get_field_id('radius'); ?>"><?php _e('Show Listings From:');?>
	 <select id="<?php echo $this->get_field_id('radius'); ?>" name="<?php echo $this->get_field_name('radius'); ?>">
          <option value="1" <?php if(attribute_escape($closer_factor)=='1'){ echo 'selected="selected"';} ?>><?php _e('1 mile');?></option>
          <option value="5" <?php if(attribute_escape($closer_factor)=='5'){ echo 'selected="selected"';} ?>><?php _e('5 miles');?></option>
          <option value="10" <?php if(attribute_escape($closer_factor)=='10'){ echo 'selected="selected"';} ?>><?php _e('10 miles');?></option>
          <option value="100" <?php if(attribute_escape($closer_factor)=='100'){ echo 'selected="selected"';} ?>><?php _e('100 miles');?></option>
          <option value="1000" <?php if(attribute_escape($closer_factor)=='1000'){ echo 'selected="selected"';} ?>><?php _e('1000 miles');?></option>
          <option value="5000" <?php if(attribute_escape($closer_factor)=='5000'){ echo 'selected="selected"';} ?>><?php _e('5000 miles');?></option>
	  </select>
	  </label>
	</p>
	<?php
...
class category_listing extends WP_Widget {
	...
	//Constructor
		...
		$this->WP_Widget('category_listing', __('T &rarr; Category wise listing'), $widget_ops);
	}

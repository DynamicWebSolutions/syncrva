...
class homebannerwidget extends WP_Widget {
	...
	// prints the widget
		...
		<div class="top_banner_section" style="display:none;">
...
class templ_latest_posts_list_view extends WP_Widget {
		...
		global $thumb_url,$wpdb;
			...
			$this->WP_Widget('latest_posts_list_view',apply_filters('templ_latestpost_with_img_widget_title_filter',__('T &rarr; Featured Listings Home Page(List)','templatic')), $widget_ops);
		}
		...
		// prints the widget
			...
			<div id="post_<?php the_ID(); ?>" <?php if((get_post_meta($post->ID,'is_featured',true) == 1) && (get_post_meta($post->ID,'featured_type',true) =="both" || get_post_meta($post->ID,'featured_type',true) =="h")){ post_class('post featured_post');} else { post_class('post');}?>>
				<div class="post-content">
					...
					<?php if(get_post_meta($post->ID,'featured_type',true) =="both" || get_post_meta($post->ID,'featured_type',true) =="h"){?>
...
/* Latest Places/Events Custom Taxonomy  -->  Gridview BOC*/
 class templ_latest_posts_grid_view extends WP_Widget {
		...
		global $thumb_url;
			...
			$this->WP_Widget('latest_posts_grid_view',apply_filters('templ_latestpost_with_img_widget_title_filter',__('T &rarr;Featured Listings Home Page(Grid)','templatic')), $widget_ops);
		}
	...
	// prints the widget
	 ...
	 ?>
         <?php
					...
					add_action('posts_where','upcoming_events_where');
					$args=
					...
					if($upcoming->have_posts())
					{
						if(function_exists('icl_register_string')){
							icl_register_string('templatic',$more_title,$more_title);
							$more_title = icl_t('templatic',$more_title,$more_title);
						}
					?>
					<div class="loop grid clear">					
...
<?php
	...
	//widgetform in backend
		...
		if(function_exists('icl_register_string')){
			$location = icl_register_string('templatic',$more_title,$more_title);
		}
...
<?php $user_photo = get_user_meta($comm->user_id,'user_photo',true); if($user_photo != '')  { ?>
 ...
 <p> <span> <?php echo $url; ?> <?php _e('by','templatic');?> <?php echo $comm->comment_author;?></span>
		<?php  if(get_option('ptthemes_disable_rating') != 'yes'){ ?>
		 <span class="rating">
...
class eventwidget extends WP_Widget {
	...
	// prints the widget
		...
		$my_post_type = empty($instance['post_type']) ? 'post' : apply_filters('widget_post_type', $instance['post_type']);
 		$category1 = empty($instance['category']) ? '' : apply_filters('widget_category', $instance['category']);
		...
		if($my_post_type !='post'){
				...
				if(!empty($category1)){
				  ...
				  'tax_query' => array( array('taxonomy' => $taxonomy_slug,'field' => 'id','terms' => $category1,'operator'  => 'IN') ),
					'orderby' => 'ID',
					'order' => 'DESC');
				}else{
					$args = array('post_type' => $my_post_type,'posts_per_page' =>  $number	,'post_status' => array('publish'),				 
...
class widget_listing_link extends WP_Widget {
    ...
    if($current_user->ID)
    {
		...
		$redirect = $site_url.$op.'ptype=login';
		?>
		<li><a href="<?php echo $site_url; ?>/?ptype=profile<?php echo $lang_url; ?>"><?php _e('Edit profile','templatic');?></a></li>
		<li><a href="<?php echo wp_logout_url(home_url()); ?>"><?php _e('Log out','templatic');?></a></li> 
		<?php	
    }else
    { 
		if ( get_option('users_can_register') ) { ?>
		<li><a href="<?php echo $site_url; ?>/?ptype=register<?php echo $lang_url; ?>"><?php _e('Register','templatic');?></a></li> <?php } ?>
		<li><a href="<?php echo $site_url; ?>/?ptype=login<?php echo $lang_url; ?>  "><?php _e('Sign In','templatic');?></a></li> 
		<?php	
    }
	if(get_option('ptthemes_add_place_nav') == 'Yes'){
    ?>
    <?php if (!is_multisite() && function_exists('icl_t') && $_REQUEST['lang'] !=''): ?>
	     <li><a href="<?php echo $site_url; ?>/?ptype=post_listing&lang=<?php echo ICL_LANGUAGE_CODE; ?> "><?php _e('Add place','templatic');?></a></li> 
    <?php else: if($_REQUEST['lang'] !=''){ ?>
			<li><a href="<?php echo $site_url; ?>/?ptype=post_listing&lang=<?php echo ICL_LANGUAGE_CODE; ?>"><?php _e('Add place','templatic');?></a></li> 
		 <?php }else{ ?>
	...
	<?php } 
	 if(get_option('ptthemes_add_event_nav') == 'Yes') { ?>
     	<?php if (!is_multisite() && function_exists('icl_t') && $_REQUEST['lang'] !=''): ?>
		     <li><a href="<?php echo $site_url; ?>/?ptype=post_event&lang=<?php echo ICL_LANGUAGE_CODE; ?>"><?php _e('Add event','templatic');?></a></li>
        <?php else: if($_REQUEST['lang'] !=''){ ?>
					<li><a href="<?php echo $site_url; ?>/?ptype=post_event&lang=<?php echo ICL_LANGUAGE_CODE; ?>"><?php _e('Add event','templatic');?></a></li>
					<?php }else{ ?>
...
if(!class_exists('templ_facebook_fanbox'))
{
	class templ_facebook_fanbox extends WP_Widget {
		...
		// prints the widget
			...
			{
			?>		 
				<div id="fb-root"></div><script src="http://connect.facebook.net/en_US/all.js#xfbml=1&status=0"></script><fb:like-box href="<?php echo $facebook_page_url; ?>" width="<?php echo $width; ?>" show_faces="<?php echo $face; ?>" border_color="" stream="<?php echo $stream; ?>" header="<?php echo $header; ?>"></fb:like-box>
...
class templ_latest_posts_widget extends WP_Widget {
		...
		// prints the widget
			...
			if( $my_query->have_posts() ) {
	 ?>
			<div class="loop <?php echo $view; ?> clear">
			<?php if($title){ ?> 
				<h3><span><?php _e($title,'templatic');?></span>
				<?php if($link){?><a href="<?php _e($link,'templatic');?>" class="more" ><?php _e($text,'templatic');?></a><?php }?></h3> <?php }?>

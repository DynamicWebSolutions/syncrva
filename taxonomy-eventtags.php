	...
	?>
     ...
     <!--  Post Title Condition for Post Format-->
		...
		<?php
				...
				<p class="timing"> <span><?php _e('Date :','templatic');?></span> <?php echo date(get_option('date_format'),strtotime(get_post_meta($post->ID,'st_date',true))).' to '.date(get_option('date_format'),strtotime(get_post_meta($post->ID,'end_date',true)));?><br> <span><?php _e('Timing :','templatic');?></span> <?php echo get_post_meta($post->ID,'st_time',true).' to '.get_post_meta($post->ID,'end_time',true);?> </p>

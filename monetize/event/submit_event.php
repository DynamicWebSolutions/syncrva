...
$ip_status = '';
$zooming_factor = '';
add_action('wp_head', 'allow_contributor_uploads');
...
add_filter('media_view_strings','remove_media_strings');
if(isset($_REQUEST['pid']) )
{
	if(!$current_user->ID)
	{
		wp_redirect(get_settings('home').'/index.php'.$op.'ptype=login');
		exit;
	}
	$pid = $_REQUEST['pid'];
...
if($_REQUEST['ptype']=='post_event' && is_stringonly())
{
	if(@$_REQUEST['pid'])
	{
		if($_REQUEST['renew'])
 ...
 <?php if(is_allow_user_register()){?>
			...
			if($current_user->ID=='')
			 ...
			 <h5 class="form_title spacer_none"><?php _e(LOGINORREGISTER);?>  </h5>
			  ...
			  <?php global $General,$wpdb,$site_url;
					if(strstr($site_url,'?') ){ $op= "&"; }else{ $op= "?"; } ?>
              <div class="login_submit clearfix" id="login_user_frm_id">
              <form name="loginform" id="loginform" action="<?php echo get_ssl_normal_url(get_settings('home').'/index.php'.$op.'ptype=login&ptype1='.$_REQUEST['ptype']); ?>" method="post" >
              <div class="form_row clearfix">
			  ...
			  <?php	global $site_url; $login_redirect_link = $site_url.$op.'ptype=post_event';?>
	...
	dynamic_sidebar('below_registration'); } ?></div>
             ...
             <?php }?>
			  ...
			  if(strstr($site_url,'?') ){ $op= "&";}else{ $op= "?"; }
			   if(isset($_REQUEST['renew']) && $_REQUEST['renew']!= '')
				$form_renew_action_url = $site_url.$op.'ptype=post_event&renew=1&pid='.$_REQUEST['pid'];
			else
				$form_renew_action_url  = $site_url.$op.'ptype=post_event';
			 if(@$_REQUEST['pid'] || @$_POST['renew']){
				$form_action_url = $site_url.$op.'ptype=preview_event';
			 }else
			 {
				 $form_action_url = get_ssl_normal_url($site_url.$op.'ptype=preview_event',@$_REQUEST['pid']);
			 }
            ...
            <?php if(((isset($_REQUEST['pid']) && $_REQUEST['pid'] != '') && (!isset($_REQUEST['renew']) && $_REQUEST['renew'] == '')) ) { ?>
             <div class="form_row clearfix" >
             	<label><?php echo EVENT_CITY_TEXT;?> </label>
             	<?php  if($post_city_id){$city_id = $post_city_id;} else if(isset($_REQUEST['post_city_id']) && $_REQUEST['post_city_id'] != '0') {$city_id = $_REQUEST['post_city_id'];} else {$city_id = $_SESSION['multi_city'];}
				echo get_multicit_select_dl('post_city_id','post_city_id',$city_id,' class="textfield textfield_x" ');?>
		...
		echo "<div class='form_row clearfix'><label>Categories</label>";
			$sep = ",";
 			for($c=0 ; $c < count($cat_array) ; $c++){ 
				$term = get_term($cat_array[$c],'eventcategory'); 
				if($cat_array[$c] !=''){
					if((count($cat_array) - 2 ) == $c)
					{
						$sep = "";
					}?>
				<?php echo $term->name.$sep;?>
			<?php }
			} 	echo "</div>"; ?>
...
<?php 		} 
			...
			<?php } ?>
			   ...
			   <div class="form_row clearfix">
				...
				<span id="cat_price"><?php if($catid != "") { $catprice = $wpdb->get_row("select * from $wpdb->term_taxonomy tt,  $wpdb->terms t where tt.term_id = '".$catid."' and tt.term_id = t.term_id"); if($catprice->term_price !=""){ echo number_format($catprice->term_price,2); }else{ echo number_format(0,2); } }else{ if(@$cat_price1 !=""){ echo number_format($cat_price1,2);}else{ echo number_format(0,2); } } ?></span><?php if($position == '3'){ echo $currency; }else if($position != 1 && $position != 2 && $position !=3){ echo ' '.$currency; } ?>
				+ 
				<?php if($position == '1'){ echo $currency; }else if($position == '2'){ echo $currency.' '; } ?>
				<span id="pkg_price"><?php if(isset($price_select) && $price_select !=""){ echo number_format($packprice,2); } else{ echo number_format(0,2);}?></span>
				<?php if($position == '3'){ echo $currency; }else if($position != 1 && $position != 2 && $position !=3){ echo ' '.$currency; } ?>
				+ 
				<?php if($position == '1'){ echo $currency; }else if($position == '2'){ echo $currency.' '; } ?>
				<span id="feture_price"><?php if(@$fprice !=""){ echo number_format($fprice,2) ; }else{ echo number_format(0,2); }?></span>
				<?php if($position == '3'){ echo $currency; }else if($position != 1 && $position != 2 && $position !=3){ echo ' '.$currency; } ?>
				= 
				<?php if($position == '1'){ echo $currency; }else if($position == '2'){ echo $currency.' '; } ?>
				<span id="result_price"><?php if(@$total_price != ""){ echo number_format($total_price,2); }else if($catid != ""){  echo number_format($catprice->term_price,2); }else{ echo number_format(0,2);} ?></span>
				<?php if($position == '3'){ echo $currency; }else if($position != 1 && $position != 2 && $position !=3){ echo ' '.$currency; } ?>

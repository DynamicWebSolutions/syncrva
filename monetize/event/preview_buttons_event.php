...
{
	$form_action_url = $site_url.$op.'ptype=paynow_event';
}else
{
	$form_action_url = get_ssl_normal_url($site_url.$op.'ptype=paynow_event');
}
?>
...
?>
<a href="<?php echo $site_url.$op;?>ptype=post_event&backandedit=1<?php if($_REQUEST['pid']){ echo '&pid='.$_REQUEST['pid'];}?><?php if($_REQUEST['renew']){echo '&renew=1';}?>" class="b_goback fl" ><?php _e(PRO_BACK_AND_EDIT_TEXT);?></a>

...
if(isset($_REQUEST['pid']) && $post_author_id == $current_user->ID)
{
	if(!$current_user->ID)
	{
...
<script type="text/javascript" src="<?php bloginfo('template_directory'); ?>/monetize/place/place.js"></script>
	...
	if (window.XMLHttpRequest)
		...
		xmlhttp.onreadystatechange=function()
	  {
		...
		if((document.getElementById('featured_h').checked== true) && (document.getElementById('featured_c').checked== true))
		{
			...
			document.getElementById('total_price').value = parseFloat(myStringArray[0]) + parseFloat(document.getElementById('feture_price').innerHTML) +  parseFloat(document.getElementById('cat_price').innerHTML) + parseFloat(myStringArray[4]);

			document.getElementById('result_price').innerHTML = parseFloat(myStringArray[0]) + parseFloat(document.getElementById('feture_price').innerHTML) +  parseFloat(document.getElementById('cat_price').innerHTML) + parseFloat(myStringArray[4]);
			...
			document.getElementById('total_price').value = parseFloat(myStringArray[1]) + parseFloat(document.getElementById('feture_price').innerHTML) +  parseFloat(document.getElementById('cat_price').innerHTML) + parseFloat(myStringArray[4]);

			document.getElementById('result_price').innerHTML = parseFloat(myStringArray[1]) + parseFloat(document.getElementById('feture_price').innerHTML) +  parseFloat(document.getElementById('cat_price').innerHTML) + parseFloat(myStringArray[4]);
		}else{
...
<div class="steps">
	<span class="current"><!--<?php _e(ENTER_PLACE,'templatic'); ?>-->Add Group Details</span>
	<span><!--<?php _e(PREVIEW_PLACE,'templatic'); ?>-->Preview &amp; Payment</span>
	<span><!--<?php _e(SUCCESS_PLACE,'templatic'); ?>-->Group Successful</span>
</div>
...
<div class="<?php templ_content_css();?>">
	<div class="content-title">Add Group<?php // echo $page_title; ?></div>
	<?php
	...
	<?php if(isset($_REQUEST['emsg']) && $_REQUEST['emsg']==1) { ?>
	...
	<?php if($current_user->ID=='') {	?>
			...
			<div class="login_submit clearfix" id="login_user_frm_id">
				...
				<div id="componentDiv"><?php if(is_plugin_active('gigya-socialize-for-wordpress/gigya.php') && get_option('users_can_register')){ dynamic_sidebar('below_registration'); } ?></div>
				<!--End of plugin code-->
			</div>
			<?php } ?>
			<?php } ?>
			...
			<?php
			 ...
			 if($cat_display == 'select') { ?>
			 	<form name="categoryform" id="categoryform" action="<?php echo site_url().'/?ptype=post_listing'; ?>" method="post" >
			 	<?php if($current_user->ID=='')	{ ?>
			  ...
			  <h5 class="form_title ">Enter group details</h5>
				...
				<div class="form_row clearfix" >
					...
					<?php
						...
						} else if(isset($_REQUEST['post_city_id']) && $_REQUEST['post_city_id'] != '0') {
							$city_id = $_REQUEST['post_city_id'];
						} else {
							$city_id = $_SESSION['multi_city'];
						}
					?>

					<?php echo get_multicit_select_dl('post_city_id','post_city_id',$city_id,' class="textfield textfield_x" '); ?>
				</div>
...
<?php 			} ?>
				...
				<h5 class="form_title">Add group details<?php // _e(LISTING_DETAILS_TEXT);?> </h5>
				<div class="form_row clearfix" >
					<label><?php echo EVENT_CITY_TEXT;?> </label>
...
<?php 		} else {
        ...
        <input type="hidden" name="user_fname" id="user_fname_hidden" value="<?php echo $_REQUEST['user_fname'];?>" />
				...
				<input type="hidden" name="category" value="<?php if(isset($_SESSION['place_info']['category']) && $_SESSION['place_info']['category'] != '') { echo $_SESSION['place_info']['category'];} else { echo $cat[0]; }?>" />
<?php 		}
		}

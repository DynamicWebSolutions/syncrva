 ...
 if(is_allow_user_register()){?>
			...
			if($current_user->ID=='') {	 ?>
			 ...
			 <h5 class="form_title spacer_none"><?php _e(LOGINORREGISTER,'templatic');?>  </h5>
			  ...
			  <?php	global $site_url; $login_redirect_link = $site_url.$op.'ptype=post_listing';?>
             ...
             <?php }?>
			  ...
			  if(isset($_REQUEST['renew']) && $_REQUEST['renew']!= '')
				$form_renew_action_url = $site_url.$op.'ptype=post_listing&renew=1&pid='.$_REQUEST['pid'];
			else
				$form_renew_action_url  = $site_url.$op.'ptype=post_listing';
			 if(isset($_REQUEST['pid']) || isset($_POST['renew'])){
					$form_action_url = $site_url.$op.'ptype=preview';
			 ...
			 }else
			 {
				 ...
				 $form_action_url = get_ssl_normal_url($site_url.$op.'ptype=preview',$reqPid);
		...
		if(((isset($_REQUEST['pid']) && $_REQUEST['pid'] != '') && (!isset($_REQUEST['renew']) && $_REQUEST['renew'] == '')) && $cat_display == 'select') {
			...
			$sep = ",";
 			for($c=0 ; $c < count($cat_array) ; $c++){ 
				$term = get_term($cat_array[$c],'placecategory'); 
				if($cat_array[$c] !=''){
